...
class DoctorUITests(unittest.TestCase):
    def test_default_doctor_view_does_not_repeat_backend_auth_notes(self) -> None:
        checks = [
            ...
            DoctorCheck("entrypoint", "ok", "argv0=/tmp/repo/.venv/bin/menisik | python=/usr/bin/python3 | cli=/tmp/repo/.venv/bin/menisik | root=/tmp/repo"),
            ...
            DoctorCheck("storage-mode", "ok", "mode=persistent home=/tmp/.menisik"),
            DoctorCheck("storage-state", "ok", "state=/tmp/.menisik/state"),
            DoctorCheck("storage-cache", "ok", "cache=/tmp/.menisik/cache"),
            DoctorCheck("storage-artifacts", "ok", "artifacts=/tmp/.menisik/artifacts"),
            DoctorCheck("storage-config", "ok", "config=/tmp/.menisik/config"),
            ...
            DoctorCheck("openclaw-skill", "ok", "/tmp/.openclaw/skills/menisik/SKILL.md"),
            DoctorCheck("openclaw-wrapper", "ok", "/tmp/.openclaw/tools/menisik.py"),
            DoctorCheck("openclaw-mcp", "ok", "/tmp/.openclaw/openclaw.json → mcp.servers.menisik"),
            DoctorCheck("hermes-mcp", "ok", "/tmp/.hermes/config.yaml → mcp_servers.menisik"),
